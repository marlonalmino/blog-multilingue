...
import { getCsrf, register, notifyAuthChanged } from "../../../../lib/auth";
...
export default function SignupPage() {
  ...
  return (
    <div className="mx-auto max-w-md">
      <h1 className="mb-6 text-2xl font-semibold">{dict.auth.signupTitle}</h1>
      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        <Input label={dict.auth.username} value={username} onChange={(e) => setUsername(e.target.value)} required />
        <Input label={dict.auth.email} type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
